...
std::list<std::unique_ptr<MuxerListener>> CreateHlsListenersInternal(
  ...
  const int32_t& order_number = stream.order_number;
  ...
  listeners.emplace_back(new HlsNotifyMuxerListener(
      playlist_name, !kIFramesOnly, name, group_id, characteristics, order_number, notifier));
  if (!iframe_playlist_name.empty()) {
    listeners.emplace_back(new HlsNotifyMuxerListener(
        iframe_playlist_name, kIFramesOnly, name, group_id,
        std::vector<std::string>(), order_number, notifier));
  }
  return listeners;
}
}  // namespace
